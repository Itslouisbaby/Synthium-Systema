...
describe('PR11 v1/v2 shadow comparison harness', () => {
  it('runs v1 and v2 in parallel and returns parity evidence metadata', async () => {
    ...
    expect(result.v2Output.length).toBeGreaterThan(0);
    ...
    expect(result.evidence.outputReadyCount).toBeGreaterThanOrEqual(1);
    expect(result.evidence.outputSentCount).toBeGreaterThanOrEqual(1);
    expect(result.evidence.outputPublicationReliability).toBeGreaterThan(0);
    expect(result.evidence.outputPublicationReliability).toBeLessThanOrEqual(1);
