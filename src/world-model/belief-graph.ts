...
export class BeliefGraphManager {
  ...
  private createEmptyGraph(sessionKey: SessionKey): BeliefGraph {
    ...
    (graph as any).versionHash = this.computeGraphHash(graph);
    return graph;
  }
  ...
  addBelief(
    ...
    const timestamp = Date.now();
    const beliefId = deterministicId.generateMemoryId('belief', timestamp, belief.statement || 'unknown');
  ...
  private updateGraph(sessionKey: SessionKey, updated: BeliefGraph): void {
    // Only increment version if it hasn't been updated already
    // (addEntity/addRelation/addBelief start from empty graph with version 1)
    const currentGraph = this.graphs.get(sessionKey);
    const newVersion = currentGraph ? currentGraph.version + 1 : updated.version;
