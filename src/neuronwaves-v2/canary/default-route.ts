...
import { applyRolloutStateToPolicy, loadRolloutState, type RolloutState } from './cohort-rollout.js';
...
export async function resolveRoutingPolicyFromEnvOrState(env: NodeJS.ProcessEnv = process.env): Promise<CanaryRoutingPolicy> {
  const envPolicy = parseRoutingPolicyFromEnv(env);
  const statePath = env.SYNTH_V2_ROLLOUT_STATE_PATH ?? '.synth/canary/rollout-state.json';
  const state = await loadRolloutState(statePath);
  return applyRolloutStateToPolicy(envPolicy, state);
}

export function shouldBlockGACutover(
  gate: CanaryGateStatus | undefined,
  env: NodeJS.ProcessEnv = process.env
): { blocked: boolean; reason?: string } {
  const guardEnabled = env.SYNTH_GA_RELEASE_GUARD === '1';
  if (!guardEnabled) {
  ...
  if (gate?.decision !== 'promote') {
    return {
      blocked: true,
      reason: `GA guard requires promote decision, received ${gate?.decision ?? 'none'}`
    };
  }
...
export async function resolveRolloutStateFromEnv(env: NodeJS.ProcessEnv = process.env): Promise<RolloutState | undefined> {
  const statePath = env.SYNTH_V2_ROLLOUT_STATE_PATH ?? '.synth/canary/rollout-state.json';
  return loadRolloutState(statePath);
}
