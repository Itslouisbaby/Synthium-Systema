/**
 * CLI Approve Command - Milestone 5
 * synth approve --session <id> --step <stepId>
 */
 ...
 */
function writeApproval(workspace: string, sessionId: string, approval: Approval): void {
...
export default async function approveCommand(options: CLIOptions): Promise<CLIResult> {
  ...
  if (!stepId) {
    return {
      exitCode: 1,
      error: 'Error: --step is required for approve command',
    };
  }
  ...
  // Write approval
  try {
    writeApproval(workspace, sessionId, {
      stepId,
      decision: 'approved',
    ...
    return {
      exitCode: 0,
      output: `Approved step ${stepId} in session ${sessionId}`,
    };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return {
      exitCode: 2,
      error: `Error: Failed to write approval: ${message}`,
    };
  }
}
