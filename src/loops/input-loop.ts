...
export class InputLoop implements MicroLoop {
   ...
   */
  private normalizeInput(input: ExternalInput, sessionKey: SessionKey): Array<Omit<Signal, 'signalId'> & { signalId?: string }> {
    const signals: Array<Omit<Signal, 'signalId'> & { signalId?: string }> = [];
